...
public class EmergencyResource {
    ...
    public ResponseEntity<Emergency> updateEmergency(
        ...
        // the document wholesale, so without this any caller who may edit a Emergency could archive or
        ...
        emergency.setArchivedAt(existing.getArchivedAt());

        emergency.setArchivedById(existing.getArchivedById());

        emergency.setArchiveReason(existing.getArchiveReason());
    ...
    /**
     * {@code POST /api/emergencies/:id/archive} : retire a emergency from the working lists.
     *
     ...
     * a discipline that may not write a emergency must not be able to retire one either.</p>
     *
    ...
    public ResponseEntity<Emergency> archiveEmergency(
        @PathVariable("id") String id,
        @RequestBody(required = false) Map<String, String> body
    ) {
        log.debug("REST request to archive Emergency : {}", id);
        patientScope.requireWrite(ClinicalDomain.ENCOUNTER);
        ...
        Emergency archived = emergencyService.archive(id, professionalId(), reason.trim());
        return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, id)).body(archived);
    }

    /**
     * {@code POST /api/emergencies/:id/unarchive} : put a emergency back.
     *
    ...
    public ResponseEntity<Emergency> unarchiveEmergency(@PathVariable("id") String id) {
        log.debug("REST request to unarchive Emergency : {}", id);
        patientScope.requireWrite(ClinicalDomain.ENCOUNTER);
        if (emergencyRepository.findById(id).filter(current -> patientScope.isVisible(current.getPatientId())).isEmpty()) {
            return ResponseEntity.notFound().build();
        }
        Emergency restored = emergencyService.unarchive(id);
