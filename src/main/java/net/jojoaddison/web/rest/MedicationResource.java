...
public class MedicationResource {
    ...
    public ResponseEntity<Medication> updateMedication(
        ...
        // the document wholesale, so without this any caller who may edit a Medication could archive or
        ...
        medication.setArchivedAt(existing.getArchivedAt());

        medication.setArchivedById(existing.getArchivedById());

        medication.setArchiveReason(existing.getArchiveReason());
    ...
    /**
     * {@code POST /api/medications/:id/archive} : retire a medication from the working lists.
     *
     ...
     * a discipline that may not write a medication must not be able to retire one either.</p>
     *
    ...
    public ResponseEntity<Medication> archiveMedication(
        @PathVariable("id") String id,
        @RequestBody(required = false) Map<String, String> body
    ) {
        log.debug("REST request to archive Medication : {}", id);
        patientScope.requireWrite(ClinicalDomain.MEDICATION);
        ...
        Medication archived = ArchiveSupport.archive(
    ...
    /**
     * {@code POST /api/medications/:id/unarchive} : put a medication back.
     *
    ...
    public ResponseEntity<Medication> unarchiveMedication(@PathVariable("id") String id) {
        log.debug("REST request to unarchive Medication : {}", id);
        patientScope.requireWrite(ClinicalDomain.MEDICATION);
        if (medicationRepository.findById(id).filter(current -> patientScope.isVisible(current.getPatientId())).isEmpty()) {
            return ResponseEntity.notFound().build();
        }
        Medication restored = ArchiveSupport.unarchive(
            medicationRepository.findById(id),
            id,
            ENTITY_NAME,
            "medication",
